...
void Utils::sig_handler(int sig) {
    // 为保证函数的可重入性，保留原来的errno （可重入性：表示中断后再次进入该函数，环境变量与之前相同，不会丢失数据)
...
// 设置信号,在WebServer::eventListen()中调用
void Utils::addsig(int sig, void(handler)(int), bool restart) {
    // 创建sigaction结构体变量
    struct sigaction sa;
    // 将sa中所有字节设置为‘\0’ （‘\0’是字符串结束标志位）
...
// 定时处理任务，重新定时以不断触发SIGALRM信号，在WebServer::eventLoop()中调用
