...
// 异步需要设置最大阻塞队列 max_queue_size 的长度，同步默认为0,不需要设置
bool Log::init(const char *file_name, int close_log, int log_buf_size, int split_lines, int max_queue_size) {
    ...
    if (max_queue_size >= 1) {
        ...
        pthread_t tid;
        // flush_log_thread为回调函数,这里表示创建一个线程，从阻塞队列中取出内容写入日志
    ...
    // 自定义日志名，如果输入的文件名没有 / ，则直接将 时间+文件名 作为日志名
    if (p == NULL) {
