...
void WebServer::eventListen() {
    ...
    assert(m_listenfd >= 0); // assert()作用是如果它的条件返回错误，则终止程序执行

    // 是否优雅关闭连接
    if (0 == m_OPT_LINGER) { // 不使用优雅关闭
    ...
    m_epollfd = epoll_create(5); // 创建一个epoll的文件描述符
    assert(m_epollfd != -1);
...
// 处理定时器，调用回调函数删除非活动连接，将定时器从链表上删除
